...
- `memberDocs/<membre>` = { docs: { imatge, dades, autoritzacio: { v, at, file } }, fees: { '2026-27': { paid, amount, date, method, note } } }: documents i quotes. Els llegeixen l'administració, la secretaria, la gerència, la direcció i la persona; els escriuen les tres primeres. Els documents signats van a `memberFiles/<id>_<n>`. Cadascú respon els drets d'imatge i la protecció de dades a «La meva fitxa» (`profiles.imageOk`, `dataOk`, `consentAt`).
- Personal › Plantilla: «Exporta a Excel» amb secció, veu, alta, antiguitat, correu, telèfon, talla, emergència, documents i quota.
...
- **Cartells**: `production.poster` és la imatge reduïda (JPEG d'uns 800 px, com el logotip); surt a Inici, al calendari i a la fitxa del concert.
